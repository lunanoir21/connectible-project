...
>
> This directory is a read-only snapshot of the Flutter (`mobile/`)
> client, taken before the mobile app was rewritten from scratch in
> React Native + Expo (`mobile-rn/`). That was a radical framework
> change, not an incremental refactor, so this copy is kept
> permanently as the **restore/diff point** if the rewrite is ever
> judged the wrong call. Diff against it when porting to make sure an
> edge case (a `crc32.dart` fix, a `reconnect_backoff.dart` curve, a
> detail in the radar `CustomPainter`) is not silently lost.
>
> - The live, evolving mobile app is `mobile-rn/`. See
>   [`mobile-rn/docs/RULES.md`](../mobile-rn/docs/RULES.md) for the
>   rewrite rules and [`mobile-rn/docs/TASKS.md`](../mobile-rn/docs/TASKS.md)
>   for the plan.
> - This backup does **not** get deleted, even after cutover, even if
>   the rewrite is judged a success. Removing it is a deliberate call
>   for the user to make later, not part of the migration.
> - Build artifacts (`build/`, `.dart_tool/`, Gradle caches, IDE
>   files) were intentionally excluded from this snapshot; it is the
>   source tree only.
