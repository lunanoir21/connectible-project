...
/// Forwarded-notification feed (T-040). Content only ever flows phone ->
/// desktop (this device cannot compose a new notification for the peer),
/// but a dismiss here now syncs back (T-K5): it clears the entry on this
/// device immediately and relays the dismissal to the phone, clearing
/// the real system notification there too.
export function NotificationsPanel({ notifications, loading, loadError, onRefresh }: NotificationsPanelProps) {
  ...
  const dismiss = (notificationId: string) => {
    void ipc.dismissNotification(notificationId);
  };
  ...
  return (
    <section className="flex h-full flex-col gap-5 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <span className="pill bg-surface-raised text-ink-faint">{t("notifications.mirrored")}</span>
      </div>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-ink-faint transition-colors hover:bg-white/[0.06] hover:text-ink"
                  >
